<template>
  <b-container>
    <b-row class="m-3 justify-content-center"
      ><img src="/me.png" height="240px"
    /></b-row>
    <b-row>
      <div class="go bb righteous">AMDP</div>
      &nbsp;&nbsp;
      <div class="base jj f">
        Hi, I am <b>A</b>lessandro <b>M</b>erletti <b>D</b>e <b>P</b>alo, film
        director and researcher in cooperation science.<br />
      </div>
    </b-row>
    <br />
    <b-row>
      <div class="go bb righteous">FILM DIRECTION</div>
      &nbsp;&nbsp;
      <div class="base jj f">
        As a film director, I co-founded with Giovanni Caloro
        <a href="http://amarenapictures.com">Amarena Pictures</a>, an
        audiovisual production company born as a service for our directions in
        2010.
      </div>
      <br />
      <b-row>
        <div class="go bb righteous">COOPERATION SCIENCE</div>
        &nbsp;&nbsp;
        <div class="base jj f">
          I co-founded with Ilario Tito and some other girls and guys
          <a href="http://cooperacy.org">Cooperacy</a>, a research organization
          about Cooperation Science, with the purpose of co-creating a social
          ecosystem based on proportional distribution of activities, profits
          and governance. <br />I support complex organizations coordination,
          analyse their level of cooperation, hold conferences and workshops
          about Cooperation Science.<br />
        </div>
      </b-row>
      <b-row class="w-100 justify-content-center m-3">
        <b-col cols="12" class="text-center">
          Hoping you found good informations, I wish you a pleasant and happy
          day.
        </b-col>
      </b-row>
    </b-row>
  </b-container>
</template>

<!--
Traits:<br /><br />
HSP: HSP-E <br />
https://hsperson.com/test/highly-sensitive-test<br />
16P: ENFJ-ENFP-A <br />
https://www.16personalities.com<br />
Human Design ManGen 1/3-1/4 <br />
https://www.jovianarchive.com/get_your_chart<br />
5BIGPERSONALITIES: I E 78   II ES 65   III A 71  IV C 92   V I 84
https://www.truity.com/test/big-five-personality-test
LOVELANGUAGE: Quality Time 30% Physical Touch 28% Acts of service 20% Words of Affirmation 18% Receiving gifts 4%
https://www.5lovelanguages.com/quizzes/singles-quiz<br />
-->